...
const Wallet = () => {
    ...
    const redeemNow = (e) => {
        ...
        if (Number(redeemInput) < minimumValue()) {
            setModal(false);
            toast.error(t(`Minimum redeemable amount is ${currency_symbol[0]?.message}${minimumValue()}`));
            return;
    ...
    return (
        <>
                                                ...
                                                {paymentData.length > 0 ? (paymentData.map((data, index) => (
                                                    <div className="reedem_request" key={index}>
                                                        <div className="redeem">
                                                            ...
                                                            <p className="redeem_price">{currency_symbol[0]?.message}{data.payment_amount}</p>
                                                        </div>
            ...
            <Modal maskClosable={false} title={t("Wallet")} centered visible={modal} onOk={() => setModal(false)} onCancel={(e) => { setModal(false); onCancelbutton(e)}} footer={null}>
                <h4>{t("Redeemable Amount")} {currency_symbol[0]?.message}{redeemInput}</h4>
