...
import { settingsData, settingsLoaded, sysConfigdata, systemconfigApi } from "./store/reducers/settingsSlice";
...
const App = () => {
    ...
    const appdata = selectdata && selectdata.filter(item => item.type == "theme_color");

    const themeColor = appdata && appdata.length > 0 ? appdata[0].message : '';
