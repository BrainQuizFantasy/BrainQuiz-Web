...
const ContestPlayQuestions = ({ t, questions: data, timerSeconds, onOptionClick, onQuestionEnd, showLifeLine, showBookmark, showQuestions }) => {
    ...
    // extra reward 
    const extraFastAnswerReward = async () => {
        let seconds = child.current.getTimerSeconds();
        let finalSeconds = timerSeconds - seconds
        if(finalSeconds <= config.rewardfirstTwoSeconds){
            return 2 
        }else if(finalSeconds <= config.rewardfirstfourSeconds){
            return 4
        }else{
            return 0
        }
    ...
    const handleAnswerOptionClick = async(selected_option) => {
        let fastAnswer = await extraFastAnswerReward();
        ...
        if (!answeredQuestions.hasOwnProperty(currentQuestion)) {
            ...
            if (decryptedAnswer === selected_option) {
                result_score++;
                setScore(result_score + fastAnswer);
                rightclick.play();
            } else {
                wrongclick.play();
            }
